...
## 6. Стартовый экран (кнопочный флоу) + судьба quick-reply чипов

**Решение владельца 2026-06-12:** пока диалога нет, чат открывается стартовым экраном `components/chat/StartScreen.tsx` — НЕ сообщением в облаке:

- Hero: «Привет! Я Оплатишка» (`font-display`, крупно, по центру) + подзаголовок muted.
- Кнопка «Выбрать сервис» разворачивает грид плиток сервисов: логотип (simple-icons по slug, `components/chat/serviceLogos.tsx`; у брендов без иконки — буква на плашке `--color-paper`) + название. Последняя плитка — «Свой вариант…» (акцент): раскрывает поле ввода → обычный чат с агентом.
- Клик по сервису → панель тарифов (название · период · цена в ₽ с комиссией) или, для custom-amount сервисов вроде Airbnb, поле суммы в долларах ($1–500).
- Выбор тарифа → POST `/api/orders/propose` → карточка заказа (OrderPanel) в ленте → существующий confirm/payment. **Весь happy path — ноль AI-токенов**: каталог и цены из `GET /api/catalog` (источник — `pricing_policy`, поддерживает владелец), агент только для «Своего варианта».
- Поле ввода чата на стартовом экране скрыто; раскрывается «Своим вариантом», ошибкой кнопочного флоу или первым заказом/сообщением.

История quick-reply чипов: убраны 2026-06-09 (контекстные подсказки над полем ввода); компонент `QuickReplyChip` живёт в барреле `components/comic` (styleguide). Контекстные чипы над инпутом по ходу диалога по-прежнему не размещать без нового решения владельца.
