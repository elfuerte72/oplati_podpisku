...
oplati_podpicky/
├── apps/
│   └── web/                         Next.js 16 (App Router, Tailwind v4, TS strict) — единый деплой: лендинг, /api/health, в будущем бот/чат/админка
│       ├── app/
│       │   ├── layout.tsx           RSC root layout (lang=ru, metadata)
│       │   ├── page.tsx             Лендинг-плейсхолдер
│       │   ├── globals.css          Tailwind v4 base
│       │   └── api/health/route.ts  Healthcheck (GET → {status, env, timestamp})
│       ├── lib/
│       │   ├── env.ts               Zod-валидация server/client env (lazy)
│       │   ├── env.server.ts        `import 'server-only'` re-export
│       │   ├── logger.ts            pino + redact + child-logger
│       │   ├── sentry.ts            beforeSend PII-scrubber
│       │   └── supabase/            browser + server + admin (service_role) clients
│       ├── sentry.{client,server,edge}.config.ts
│       ├── instrumentation.ts       server/edge Sentry + fail-fast env
│       ├── instrumentation-client.ts
│       ├── next.config.ts           transpilePackages + withSentryConfig
│       └── package.json
├── packages/
...
| File | Purpose |
|------|---------|
| `apps/web/app/layout.tsx` | RSC root layout (ru, metadata) |
| `apps/web/app/page.tsx` | Лендинг-плейсхолдер |
| `apps/web/app/api/health/route.ts` | Healthcheck (liveness) |
| `apps/web/lib/env.ts` | Zod server/client env, lazy-парсинг |
| `apps/web/lib/logger.ts` | pino singleton + `childLogger(module)` |
