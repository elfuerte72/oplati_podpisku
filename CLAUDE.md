...
- **Тесты**: Vitest в `apps/web` (loveandpay: client/sign/handlers; ai: бюджет/роутер; chat: toolCards; ratelimit; security/timing-safe; jobs/issue-card) и `packages/types` (state machine, схемы L&P).

## Фаза 2 — виртуальные карты (PaySpace), в работе

План фазы с шагами и открытыми решениями — [`PLAN.md`](PLAN.md).

Карты выпускает **app.pay.space** (это НЕ Love&Pay; L&P — только приём RUB). Уже в коде: таблица `cards` (enum `card_status = active/idle/recycled`), репозиторий, клиент `lib/pay-space/` (`createCard`/`topupCard`/`getCard`), job `issue-card` (вызывается из L&P-webhook; **атомарный claim `paid → in_fulfillment` ДО топ-апа** через `transitionOrderDetailed` — at-most-once, без двойной траты при гонке/повторе; затем topup активной карты → recycle чужой → выпуск новой → реквизиты клиенту в Telegram → `completed` как `system`; потерянный/повторный запуск добивает cron `poll-payment`), cron `recycle-cards` (90 дней → `idle`, 180 → `recycled`).
