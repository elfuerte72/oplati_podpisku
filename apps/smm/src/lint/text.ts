 ...
 *
 * ⚠️ Границы слова заданы lookaround, а НЕ `\b`: в JS `\b` не видит кириллицу
 * (правило CLAUDE.md), и правило, дословно перенесённое из Python, молча не
 * находило ни одного числительного.
 */
const NUMERAL_WORD_RE = new RegExp(
  '(?<![а-яёa-z])(?:дв(?:а|е|ух|ое|ум|умя|оих)|тр(?:и|ёх|ех|ое|оих|ём|ем)|четыр(?:е|ёх|ех|ём|ем)|' +
    ...
    'полов[а-я]+|четверт[а-я]+|треть[а-я]*)(?![а-яёa-z])',
  'giu',
);
 ...
 *
 * Переносы строк схлопываются ДО разбора: одно и то же предложение в разных
 * постах может быть свёрнуто по-разному, и разбиение по `\n` находило бы
 * повтор только при совпадающей вёрстке.
 */
