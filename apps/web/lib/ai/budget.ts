 ...
 *
 * Fail-open: недоступная БД не выключает агента (это не единственный слой
 * защиты — есть WAF rate-limit и prepaid-баланс Anthropic), а ошибки записи
 * не ломают ответ пользователю. Sentry-алерт уходит один раз — при пересечении
 * порога, а не на каждый отклонённый запрос.
 */
