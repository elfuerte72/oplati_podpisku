 ...
 *
 * Опрашивается именно `order`, а не `snapshot`: одна строка заказа вместо
 * всего кабинета. Снапшот перечитывается, когда статус сменился, — и уже за
 * ним идёт запрос живого баланса карты в PaySpace (`card-live`).
 */
 ...
 * `activated` Telegram. Каждый возврат перечитывает снапшот и следом без паузы
 * спрашивает живой баланс карты в PaySpace: второй вызов в том же окне — лишний.
 */
