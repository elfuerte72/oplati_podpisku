...
/**
 * Live-баланс основной карты для кабинета — действие `card-live`
 * (`buildCardLive`). Снапшот его НЕ ждёт: провайдер отвечает ~1,2 с, а снапшот
 * открывает кабинет и перечитывается после каждого листа.
 *
 ...
 * без синхронизации кабинет показывал бы завышенный баланс. Поэтому следом за
 * снапшотом кабинет спрашивает реальный баланс (`getCardInfo`) для карты,
 * которую показывает основной, а мы кэшируем его в БД — следующий снапшот уже
 * отдаёт сверенное значение.
 *
