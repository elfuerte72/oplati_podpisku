...
describe('buildPathSteps — выпуск карты', () => {
  ...
  it('у клиента уже есть карта — шаг 2 не обещает ни новую, ни ту же', () => {
    // Долить на ту же или выпустить новую, решает сервер (карта могла
    // истекать) — экран говорит только то, что верно в обоих случаях.
    const steps = buildPathSteps({ stage: 'issuing', service: 'Claude', topUp: true });
    expect(steps[1]?.title).toBe('Кладу деньги на карту');
  });
});
