...
export const ORDER_STATUS_LABELS: Record<OrderStatus, string> = {
  ...
  // Не «Готово»: карта выдана, а подписку клиент оплачивает сам на сайте
  // сервиса. «Готово» читалось как «подписка подключена» (разбор пути клиента
  // 2026-09-23); панель персонала держит свой словарь и здесь не участвует.
  completed: 'Карта выдана',
  failed: 'Ошибка',
  cancelled: 'Отменён',
  expired: 'Истёк срок',
...
export type OrderSummary = {
  ...
  /**
   * Когда заказ оплачен (ISO); `null` — оплаты не было. Нужен СВОДКЕ, а не
   * только детали: по нему вкладка «Карта» отличает «оплатил, а выдача упала»
   * от недоплаты (`isPaidButIssueFailed`). Без поля в сводке ветка
   * «Карта задерживается» была мёртвой при зелёных тестах (ревью 2026-09-25).
   */
  paidAt: string | null;
};
