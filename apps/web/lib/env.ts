...
const serverEnvSchema = z.object({
  ...
  // означать открытую панель. Не задан → в development гейт выключен (хост
  // `localhost:3000`), в production панель ЗАКРЫТА целиком — fail-closed,
  // `lib/panel/host.ts`.
  ...
  // (`orderFloorRub()` в `lib/payments/gateway.ts`: max(`PRODUCT_MIN_ORDER_RUB`, минимум АКТИВНОГО шлюза)).
  ...
  // Аварийный выключатель preflight карточного фонда: '1'/'true' — счёт
  // выставляется без проверки, что карту будет чем выпустить (поведение до
  // трека vcc-preflight). Отдельным флагом, а не нулём в пороге: прецедент
  // `VCC_BALANCE_ALERT_DISABLED` — ноль читался как «настроено» и молча
  // выключил алёрт баланса ровно до инцидента, который он должен был
  // предупредить.
  ...
  // Комиссия PaySpace за пополнение карточного счёта из крипто-кошелька
  // (`/vcc/balance/topup/`, проценты). Провайдер называет её только в ответе на
  // УЖЕ созданную заявку, поэтому для расчёта «сколько пополнить» в разделе
  // «Финансы» число живёт здесь: ~3 % по опыту владельца. Обновлять по факту
  // очередной заявки — `fee_percent` в её ответе. На деньги не влияет: это
  // подсказка на экране, а не сумма перевода.
  PAYSPACE_VCC_TOPUP_FEE_PERCENT: z.coerce.number().min(0).max(50).default(3),

  // FKWallet.io — кошелёк, через который рубли с кассы Freekassa превращаются
  // в USDT для PaySpace (единственный путь: касса выводит только на него).
  // Панель читает его баланс. ⚠️ Приватный ключ у провайдера НЕ «только
  // чтение»: тем же ключом API выводит и переводит деньги — в кабинете
  // кошелька включить защиту по IP на адрес VPS. Не заданы → раздел «Финансы»
  // пишет «не настроено», приложение работает как раньше.
  FKWALLET_PUBLIC_KEY: optionalEnvString(),
  FKWALLET_PRIVATE_KEY: optionalEnvString(),
  // Только https: подпись уходит в заголовке каждого запроса.
  FKWALLET_BASE_URL: z
    .string()
    .url()
    .startsWith('https://')
    .default('https://api.fkwallet.io/v1'),
  ...
  REFERRAL_ENABLED: z
    .preprocess((v) => v === '1' || v === 'true', z.boolean())
    .default(false),
  // Минимум ЗАЯВКИ НА ВЫВОД (USD-центы). Поднят 1000 → 1300 ($13 ≈ 1000 ₽,
  // решение владельца 2026-09-10): разрыв со списанием намеренный — списание
  // автоматическое и не стоит нам ничего, а каждая заявка на вывод это ручная
  // работа владельца, переписка о реквизитах и комиссия платёжного канала.
  REFERRAL_MIN_PAYOUT_USD_CENTS: z.coerce.number().int().positive().default(1300),

  // ─── Оплата заказа реферальными баллами (трек referral-balance-spend) ────
  //
  // Второй выключатель ПОВЕРХ `REFERRAL_ENABLED`: выключенный означает, что
  // переключателя нет на экране и роут скидку не применяет. Уже занятые баллы
  // при этом продолжают учитываться в балансе — гасить фичу не значит
  // конфисковать деньги партнёра.
  REFERRAL_SPEND_ENABLED: z
    .preprocess((v) => v === '1' || v === 'true', z.boolean())
    .default(false),
  // Минимум ОДНОГО списания (USD-центы). Скидка меньше — списания нет вовсе:
  // «сэкономили 12 ₽» не стоит ни клику клиента, ни строке в балансе.
  REFERRAL_SPEND_MIN_USD_CENTS: z.coerce.number().int().positive().default(100),
  // Премия за трату баллов, % поверх паритета: 0 — $1 балла гасит $1 по курсу
  // заказа (решение Q2), 20 — гасит $1.20. ЕДИНСТВЕННОЕ место конверсии
  // (`bonusValueKopecks`), чтобы премия включалась цифрой в env, а не второй
  // формулой в коде. Кап 100%: выше премия начала бы съедать больше маржи,
  // чем потолок «до комиссии» вообще оставляет.
  REFERRAL_SPEND_RATE_BONUS_PERCENT: z.coerce.number().int().min(0).max(100).default(0),
  // Кому фича доступна: список telegram_id через запятую. ПУСТО = всем (после
  // смоука список снимается). На время выката — id владельца: dev платёжных
  // ключей не имеет намеренно, поэтому сквозной путь проверяется на проде
  // живым заказом (решение Q14).
  REFERRAL_SPEND_ALLOWLIST: z.string().default(''),

  // ─── Промокоды (трек promo-codes) ───────────────────────────────────────
  //
  // Выключатель всей механики: выключенный означает, что поля ввода нет на
  // экране и роут оплаты скидку не применяет. Уже занятые под живыми счетами
  // промокоды при этом продолжают действовать — гасить фичу не значит
  // отказаться платить по выставленным документам.
  //
  // ⚠️ Флаг НЕ управляет тем, какие коды существуют и сколько дают: это колонки
  // `promo_codes`. Здесь только «работает механика или нет» — ровно как
  // `REFERRAL_SPEND_ENABLED` не знает про размер баланса.
  ...
  //
  // Глобальный выключатель воронки (спека .scratch/retention-funnel/, история
  // 17: гасится одним флагом без отката кода). Дефолт ВЫКЛЮЧЕНО: включается
  // на проде отдельным шагом ПОСЛЕ миграций тикета 01 (порядок — тикет 08).
  // Привратник sendFunnelMessage проверяет флаг первым — выключенный флаг
  // означает тишину, а не ошибки.
  RETENTION_FUNNEL_ENABLED: z
    .preprocess((v) => v === '1' || v === 'true', z.boolean())
    .default(false),
  // Адрес чата отзывов (https://t.me/...). Питает url-кнопку «Отзывы» в
  // /start-меню и приглашение после оценки 4–5. Не задан → кнопки нет и
  // приглашение уходит без ссылки (fail-quiet, прецедент — необязательные
  // кнопки меню).
  REVIEWS_CHAT_URL: optionalEnvString(z.string().url().startsWith('https://')),
  ...
  // ─── Помощник поддержки (DeepSeek) ──────────────────────────────────────
  //
  // ТРЕТИЙ AI-контур рядом с продажным агентом и Haiku-роутером, и намеренно со
  // своими переменными: авария или исчерпанный ключ у одного провайдера не
  // должны гасить второго, а расходы DeepSeek не участвуют в дневном бюджете
  // Anthropic (другой прайс).
  //
  // Выключено по умолчанию: при выключенном флаге кнопка «Поддержка» ведёт в
  // сегодняшний двухшаговый флоу к человеку. Выключение обязано быть безопасным.
  SUPPORT_AI_ENABLED: z
    .preprocess((v) => v === '1' || v === 'true', z.boolean())
    .default(false),
  SUPPORT_AI_API_KEY: optionalEnvString(),
  // ⚠️ Дефолтов здесь НЕТ намеренно (инвариант 10: зеркало лучше убрать, чем
  // сверять). Значения по умолчанию — `SUPPORT_AI_DEFAULT_BASE_URL` и
  // `SUPPORT_AI_DEFAULT_MODEL` в `@oplati/agent/src/client.ts`, там же, где
