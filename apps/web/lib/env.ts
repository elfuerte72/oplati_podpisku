...
const serverEnvSchema = z.object({
  ...
  // Запас под FX-конвертацию сети, foreign-fee и местный VAT: реальный charge
  // мерчанта часто выше витринной цены. Карты выпускаем американские, поэтому на
  // USD-мерчантах FX почти нулевой и хватает 10%; для не-USD мерчантов (евро-
  // подписки) запас впритык — калибровать по реальным заказам. Закладывается
  // ТОЛЬКО в сумму карты, в цену для клиента НЕ входит; неизрасходованный остаток
  // возвращается на VCC-баланс при release. 0 = карта ровно на цену.
  PAYSPACE_CARD_BUFFER_PERCENT: z.coerce.number().int().min(0).max(100).default(10),
