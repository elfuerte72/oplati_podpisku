...
export type FreekassaClientOptions = {
  ...
  /**
   * Наблюдатель за ЛЮБЫМ сбоем обращения (API-отказ, контракт-дрейф, транспорт).
   * Зовётся перед пробросом ошибки наверх, на поток не влияет и решать ничего
   * не должен — существует, чтобы клиент остался транспортом и не знал про
   * Sentry/Telegram. Единственный потребитель — ops-алёрт об отказе по `nonce`
   * (`nonce-alert.ts`), инжектится в `index.ts`.
   */
...
export class FreekassaClient {
  ...
  /**
   * Тонкая обёртка над `sendJson` ради наблюдателя `onApiError`: он обязан
   * видеть ЛЮБОЙ сбой обращения — и отказ провайдера, и контракт-дрейф, и
   * транспорт, — потому что «шлюз отвечает, но отвергает всё» снаружи выглядит
   * так же зелено, как здоровый прод. Ошибка уходит вызывающему без изменений.
   */
