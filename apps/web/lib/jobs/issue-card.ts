...
async function markOrderFailed(orderId: string, reason: string, shortId?: string): Promise<void> {
  ...
  //
  // ⚠️ Гасим и на пути `paypace_topup_pending`, где исход топапа НЕИЗВЕСТЕН
  // (деньги могли зачислиться позже). Это осознанный выбор в пользу заказа:
  // пока он в `failed`, комиссия не причитается. Если топап всё же прошёл и
  // оператор воскресил заказ (`failed → refund_requested → completed`),
  // начисление восстанавливается вручную — автоматом нельзя, повторная вставка
  // упрётся в частичный UNIQUE (см. reverseAccrualsForOrder). Альтернатива —
  // не гасить до выяснения — оставляла бы живую комиссию на заказе, который в
  // подавляющем большинстве случаев так и остаётся неисполненным.
