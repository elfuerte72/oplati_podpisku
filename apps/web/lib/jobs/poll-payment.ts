...
export async function pollPayments(): Promise<{
  ...
  if (isPaySpaceConfigured()) {
    ...
    // Опрос фонда под выпуск карт — каждые 5 минут, чтобы поймать уход баланса
    // в ноль посреди дня (пополнение VCC — T+1).
    //
    // ⚠️ Это уже НЕ только мониторинг: тот же вызов пишет снимок
    // `vcc_balance_snapshots`, из которого гейт оплаты узнаёт, есть ли чем
    // выпустить карту (трек vcc-preflight, тикет 03). Убрать его «раз алёрт не
    // нужен» — значит заставить каждую оплату ходить к провайдеру самой.
