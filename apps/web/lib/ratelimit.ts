...
const CONFIGS: Record<RateLimitName, LimiterConfig> = {
  ...
  // Потолок выше прежних 20, но умеренно: `card-live` тянет живой запрос в
  // PaySpace за балансом карты (`lib/cabinet/live-balance.ts`) и не кэшируется,
  // поэтому щедрость здесь оплачивается чужим API. Клиент зовёт его не чаще
  // раза в 15 с, кроме возврата в приложение.
  ...
  // Проверка промокода из Mini App по `telegram_id` (трек promo-codes).
  //
  // ОТДЕЛЬНЫЙ бакет от `cabinet` по двум причинам сразу. Во-первых, это
  // единственная точка, где ПОДБОР осмыслен: действующий код — это деньги, и
  // общий с листанием экранов кошелёк означал бы, что перебор либо не режется
  // вовсе, либо режется ценой неработающего кабинета. Во-вторых, обратное тоже
  // верно: клиент, пробующий код с опечатками, не должен выесть себе лимит на
  // оплату.
  //
  // 15 в минуту — человеку хватает с запасом (код вводят раз-два), а перебор
  // словаря на такой скорости бессмыслен.
  'promo-check': { limit: 15, windowSeconds: 60 },
};
