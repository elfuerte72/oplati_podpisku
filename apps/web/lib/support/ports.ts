...
export interface SupportModelPort {
  /**
   * Доступен ли помощник: флаг `SUPPORT_AI_ENABLED` И ключ
   * (`isSupportAiAvailable`). Флаг без ключа обязан вести себя как выключенный.
   *
   * ⚠️ Модуль зовётся и при недоступном помощнике: он выключатель МОДЕЛИ, а не
   * учёта. Режим разговора читается всегда — иначе реплика клиента, ответившего
   * оператору, теряет маркер обращения (crm-serious-fixes, тикет 01).
   */
  available(): boolean;
  /**
