  ...
  if (action === 'confirm') {
    ...
    try {
      ...
      if (err instanceof EmailRequiredError) {
        await sendSafely(
          chatId,
          'Почти готово! Для выставления счёта нужна почта для связи по заказу. Открой заказ в кабинете (кнопка «Открыть приложение» в /start), укажи почту на экране заказа — и оплата откроется там же.',
          updateId,
        );
        return;
      }
