...
async function buildPendingOrderHandoffText(
  ...
  try {
    ...
    if (err instanceof EmailRequiredError) {
      ...
      return (
        'Готово, Telegram привязан! Остался один шаг до оплаты: укажи почту для связи по заказу — открой заказ в кабинете (кнопка «Открыть приложение» в /start), поле почты там, на экране заказа. После этого счёт выставится там же.'
      );
    }
