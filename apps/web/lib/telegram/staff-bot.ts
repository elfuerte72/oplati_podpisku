 ...
 * у него поверхности. В группах и супергруппах молчит вовсе (туда он только
 * ШЛЁТ — уведомления в темы ops-группы).
 *
...
export async function handleStaffBotUpdate(update: TelegramUpdate): Promise<void> {
  ...
  // Отвечаем ТОЛЬКО в личке. В ops-группе (трек ops-group) бот — отправитель
  // уведомлений, и ответ «Команд у меня нет» под каждой репликой человека —
  // шум; служебные сообщения тем (создание, закрытие) тоже приходят как
  // `message`. Выход ДО лимитера и БД: на группу не тратим ни того, ни другого.
  if (message.chat.type !== 'private') {
    log.debug({ event: 'telegram.staff_bot.ignored_chat_type', chatType: message.chat.type });
    return;
  }
