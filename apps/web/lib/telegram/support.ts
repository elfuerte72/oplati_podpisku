...
export async function sendToSupportOperator(
  ...
  try {
    ...
    const res = await notifyStaff(stripHtmlTags(operatorMessage), {
      ...
      preformatted: true,
      // ⚠️ И в тему группы, И личкой (решение владельца 2026-09-09): группу
      // листают, а клиент ждёт ответа. Цена пропущенного обращения измерена —
      // 08.09 человек тринадцать часов считал нас мошенниками при живом заказе.
      alsoDirect: true,
