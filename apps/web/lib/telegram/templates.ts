...
/**
 * Подпись url-кнопки под сообщением с картой — ведёт туда, где клиент оформляет
 * купленный сервис (`payment_instructions.paymentUrl`, иначе официальный прайс).
 * Названа сервисом, а не «прайсом»: это следующий шаг клиента, а не справка.
 */
export function openServiceButton(serviceName: string): string {
  return `🌐 Открыть ${serviceName.trim()}`;
}

/**
 * Подпись web_app-кнопки под сообщением с картой (открывает кабинет, где карта
 * и её реквизиты). При пополнении реквизитов в сообщении нет — только здесь.
 */
export const CARD_IN_APP_BUTTON = '💳 Карта в приложении';
...
// ─── Сообщение с картой (issue-card) ──────────────────────────────────────
//
// Порядок блоков — ответ на вопросы клиента по очереди (разбор пути клиента
// 2026-09-23): что мне дали → что мне сделать → реквизиты → адрес → правила.
// Прежнее сообщение открывалось «Оплатить строго по цене $X» и списком правил
// без названия сервиса: только что заплативший человек читал его как «доплатите»
// и ждал, что подписку подключат за него. Страну выпуска карты не называем
// (ТЗ «клиентский путь» §2) — «США» ниже относится к VPN и адресу плательщика.

/** Название сервиса в сообщении; `null` (заказ вне каталога) → нейтральное слово. */
function serviceLabelHtml(serviceName: string | null, fallback: string): string {
  const trimmed = serviceName?.trim();
  return trimmed ? escapeHtml(trimmed) : fallback;
}

/**
 * Начало сообщения о ВЫПУСКЕ карты: что получил клиент и какой шаг за ним.
 * Вторая строка — главное, что клиент должен вынести: оплату сервису делает он
 * сам, в своём аккаунте, этой картой.
 */
export function cardIssuedIntroHtml(serviceName: string | null, priceUsdCents: number): string {
  const forService = serviceLabelHtml(serviceName, 'оплаты подписки');
  const onSite = serviceLabelHtml(serviceName, 'сервиса');
  return [
    `<b>Карта для ${forService} готова.</b>`,
    `Последний шаг делаешь ты: зайди в свой аккаунт на сайте ${onSite}, оформи подписку за ${formatUsd(priceUsdCents)} и заплати этой картой.`,
  ].join('\n');
}

/**
 * Начало сообщения о ПОПОЛНЕНИИ уже выданной карты. Два случая, и оба честно:
 * подписка уже оплачивается с этой карты — сервис спишет сам; оформляет
 * впервые — платит той же картой, реквизиты которой уже у него.
 */
export function cardToppedUpIntroHtml(serviceName: string | null, priceUsdCents: number): string {
  const forService = serviceLabelHtml(serviceName, 'оплаты подписки');
  const onSite = serviceLabelHtml(serviceName, 'сервиса');
  return [
    `<b>Карта для ${forService} пополнена — на оплату ${formatUsd(priceUsdCents)}.</b>`,
    'Если подписка уже оплачивается с этой карты, сервис спишет деньги сам — делать ничего не нужно.',
    `Если оформляешь впервые — зайди в свой аккаунт на сайте ${onSite} и заплати той же картой, что и раньше. Номер, срок и CVC — по кнопке «Карта в приложении».`,
  ].join('\n');
}

/**
 * Подсказка под реквизитами: значения в `<code>` Telegram копирует по нажатию,
 * но клиент об этом не знает и перепечатывает 16 цифр руками.
 */
export const CARD_COPY_HINT =
  'Нажми на номер, срок или CVC — значение скопируется. Карта всегда есть и в приложении — кнопка «Карта в приложении» ниже.';

/** Заголовок блока адреса: по-русски, но с термином, который клиент увидит на сайте. */
export const BILLING_ADDRESS_TITLE_HTML = '<b>Адрес плательщика (Billing address)</b>';

/**
 * Пояснение под адресом. Клиенты не понимали, чей это адрес и можно ли его
 * вводить: адрес не их, и так задумано — он закреплён за картой.
 */
export const BILLING_ADDRESS_HINT =
  'Если сайт спросит адрес плательщика, вводи этот — так и задумано: адрес закреплён за твоей картой.';

/**
 * Последняя строка сообщения с картой. Раньше здесь было «после оплаты напишите
 * сюда, проверю», а бот на свободный текст отвечает «в переписке я не отвечаю»
 * (`BOT_AI_ENABLED` выключен) — обещание, которое продукт не выполнял.
 */
export function cardMessageFooter(shortId: string): string {
  return [
...
/**
 * Блок правил оплаты картой (parse_mode HTML) для сообщения `issue-card` — и
 * при выпуске новой карты, и при пополнении. Цена подписки в долларах и
 * правила, из-за которых чаще всего не проходит оплата; подробности — на
 * странице-инструкции (url-кнопка под сообщением).
 *
...
export function paymentRulesHtml(priceUsdCents: number): string {
  return [
    '<b>Чтобы оплата прошла с первого раза</b>',
    '• плати в веб-версии сервиса — в браузере, не в мобильном приложении;',
    '• включи VPN с локацией <b>США</b> и выбери страну США на сайте сервиса;',
    `• цена на сайте должна быть в долларах — ${formatUsd(priceUsdCents)}; в евро или рублях выйдет дороже из-за налога, и денег на карте не хватит;`,
    '• если цена всё ещё не в долларах — очисти кэш этого сайта и зайди заново по VPN.',
    '',
    'Пошагово для iPhone, Android и компьютера — кнопка «Как оплатить» ниже.',
  ].join('\n');
}
...
export const SUPPORT_ASK_TEXT =
  'Опишите, пожалуйста, что случилось — одним сообщением. Если это про конкретный заказ, добавьте его номер. Я сразу передам всё оператору, и он свяжется с вами здесь.';

/** Обращение принято и ушло оператору. */
export const SUPPORT_SENT_TEXT =
  'Готово — передал оператору. Он напишет вам здесь, в Telegram, в ближайшее время. Обычно отвечаем с 10:00 до 22:00 МСК.';

/** Не удалось доставить обращение оператору. */
export const SUPPORT_FAIL_TEXT =
  'Не получилось передать оператору прямо сейчас — что-то на нашей стороне. Попробуйте ещё раз через пару минут.';
...
export const STAFF_BOT_START_TEXT =
  'Бот подключён. Сюда будут приходить уведомления панели: новые обращения, застрявшие заказы, платежи на проверке банка и низкий баланс карточного счёта.';
...
export const SILENT_TEXT_HINT =
  'В переписке я не отвечаю — оплата собирается в приложении: команда /start откроет его. Нужна помощь — нажмите кнопку ниже.';
...
export const SILENT_MEDIA_HINT =
  'Картинки и файлы я не разбираю. Оплата собирается в приложении: команда /start откроет его. Нужна помощь — нажмите кнопку ниже.';

/**
 * Клиент нажал «Написать в поддержку» в Mini App: приложение закрывается, а в
 * чат приходит это сообщение с кнопкой «Поддержка» (`sendCabinetSupportHandoff`).
 * Обращение по-прежнему создаёт только нажатие кнопки — здесь она лишь
 * оказывается у клиента под рукой, а не где-то в меню `/start` выше по чату.
 */
export const CABINET_SUPPORT_HANDOFF_TEXT =
  'Вы открыли поддержку из приложения. Нажмите кнопку ниже и опишите, что случилось, — оператор ответит здесь, в чате.';
...
// ─── Реферальная ссылка: обратная связь на /start ref_ ───────────────────────
//
// Блок стоит ВЫШЕ блока воронки намеренно: канарейка `lib/funnel/texts.test.ts`
// режет файл от заголовка следующего блока (первое вхождение его названия, поэтому
// само название здесь не повторяется) до конца и требует регистрации в реестре
// каждого экспорта ниже. Новые НЕ-воронковые тексты — сюда или выше, не в хвост.
//
// До 2026-09-05 любой заход по ссылке-приглашению получал обычное приветствие —
// и друг, и сам партнёр, открывший свою ссылку «проверить». Никто не видел, что
// приглашение сработало, и партнёры писали «ссылка не работает» при исправном
// захвате (разбор по проду: 12 из 12 новых закреплены). Ставку цифрой не
// называем — как и в реферальном касании воронки.

/** Другу, который ТОЛЬКО ЧТО закрепился за партнёром (создан по ссылке или поздний захват). */
export const REFERRAL_WELCOME_TEXT =
  'Вижу, ты по приглашению друга 🙌 Для тебя всё как обычно: те же цены и условия. А друг получит небольшой бонус с твоих оплат.';

/** Партнёр открыл СВОЮ ссылку-приглашение. */
export const REFERRAL_SELF_LINK_TEXT =
  'Это твоя собственная ссылка-приглашение 🙂 Отправь её другу: когда он откроет бота по ней и оплатит подписку, бонус придёт тебе. Ссылка и доход — в приложении, раздел «Партнёрская программа».';

/** DM партнёру о новом закреплённом друге. Без имени и id: чужие данные партнёру не показываем. */
export const REFERRAL_PARTNER_JOINED_TEXT =
  '🎉 По твоей ссылке пришёл новый друг! Как только он оплатит подписку, бонус появится в разделе «Партнёрская программа».';

// ─── Воронка обратной связи и удержания ───────────────────────────────────
//
// ДЕФОЛТЫ клиентских строк воронки живут здесь (тикет 07 воронки): голос
// маскота, «ты», инлайновых текстов в джобах нет. С панели v2 (ветка C)
// владелец правит формулировки без деплоя: переопределения лежат в таблице
// `funnel_texts`, читаются ТОЛЬКО через реестр `lib/funnel/texts.ts`
// (`getFunnelTexts` + `renderFunnelText`) — крон и колбэки констант отсюда не
// импортируют. Каждая экспортируемая строка блока обязана быть в реестре
// (канарейка `lib/funnel/texts.test.ts`); подстановки — `{service}`, `{link}`,
// заполняет код. Чек-лист домена: страна выпуска карты не называется
// («картой», не «американской»); цифры и сроки — только из кода или env; без
// обещаний; в приглашении к отзыву тема вознаграждений не существует; слово
// «холд» занято антифрод-проверкой банка и здесь не используется.
...
/** Благодарность за ответ на опрос (msg1/msg2, любые кнопки кроме «Другое» и «Непонятно»). */
export const FUNNEL_THANKS_TEXT = 'Спасибо! Теперь понятнее, что улучшать.';

/**
 * Ответ на «Непонятно, как оплатить» (msg1) и «Непонятно, как это работает»
 * (msg2). Раньше им уходило то же «Спасибо!», что всем: клиент признавался,
 * что не понял, и оставался без объяснения (разбор пути клиента 2026-09-23).
 * Три шага — те же, что в приветствии /start. Под текстом — «Открыть
 * приложение» и «Как оплатить».
 */
export const FUNNEL_UNCLEAR_REPLY_TEXT = [
  'Спасибо за честный ответ! Коротко, как это работает:',
  '',
  '1. Выбираешь сервис в приложении и платишь рублями — через СБП или российской картой.',
  '2. Я выпускаю тебе виртуальную карту с нужной суммой в долларах — она появится в приложении и придёт в чат.',
  '3. Ты сам оформляешь подписку на сайте сервиса и платишь этой картой — аккаунт остаётся твоим.',
  '',
  'Пошагово, с VPN и адресом плательщика, — по кнопке «Как оплатить». Остались вопросы — напиши /support.',
].join('\n');
...
/**
 * msg3: оценка после покупки. Имя сервиса — из заказа подстановкой `{service}`;
 * для custom-описаний — нейтральная форма без названия (описание писал клиент
 * свободным текстом, возвращать его в лоб странно). Шкала 1–5 — механика
 * кнопок `fb:rate:1..5`.
 */
export const ORDER_RATING_TEXT =
  'Получилось оплатить {service} картой? Оцени от 1 до 5 — мне это очень поможет.';

export const ORDER_RATING_TEXT_GENERIC =
  'Получилось оплатить подписку картой? Оцени от 1 до 5 — мне это очень поможет.';
...
// Оценка и заказ дублируются фактами в вызове (`funnel-callbacks.ts`): тело
// остаётся самодостаточным для теста и лога, факты — для глаз в теме.
 ...
 * подстановкой `{link}`.
 */
export const REFERRAL_NUDGE_TEXT =
