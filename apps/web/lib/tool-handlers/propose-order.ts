...
// Экспортируется, потому что об этом сроке рассказывает клиенту помощник
// поддержки: своё число в его базе знаний было бы зеркалом (инвариант 10) —
// правка здесь молча делала бы ответ помощника неверным.
export const PRICE_LOCK_TTL_HOURS = 2;
const TTL_HOURS = PRICE_LOCK_TTL_HOURS;
