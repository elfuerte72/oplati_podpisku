...
Смоук после починки (заказ ORD-MQ2YE, 851,03 ₽): подпись HMAC принята, счёт
выставлен (`provider_ref=320046046`, он же `id` в ссылке на оплату — попутно
подтвердилось совпадение), заказ переведён в `pending_payment`, срок выровнен по
инвойсу, `buyerFeePercent=6` доехал до кабинета. Вебхук, подпись MD5 уведомления
и выпуск карты остаются непроверенными: провайдер шлёт уведомление только о
фактически прошедшей оплате.
