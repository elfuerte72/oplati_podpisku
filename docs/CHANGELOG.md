...
## 2026-07-14 — Надбавка 3,5% к курсу Rapira

### Changed

- Расчётный USDT/RUB курс теперь равен `askPrice Rapira × 1,035`; результат фиксируется с четырьмя знаками и используется единообразно в каталоге и заказах.
- Формула цены: `USD × расчётный курс + 30% комиссии`, затем разовые `$4` по тому же расчётному курсу, только если у пользователя нет активной карты. К аварийному `RATE_FALLBACK_USDT_RUB` также применяется 3,5%.
