...
- **Кабинет партнёра упрощён с 5 экранов до 2** (`PartnerCabinet.tsx`): всё на одном дашборде (ссылка-приглашение, сеть рефералов, спринт/ставка, доход по месяцам, путь к статусу, полная история — прокруткой) + отдельная страница «Как это работает». Экраны network/link/history/stats как отдельные табы удалены.
...
- **Презентация партнёрской программы обновлена под одноуровневую** (`public/partner-presentation.html`, `noindex`): убраны дерево на 3 уровня, ставки L2/L3 и командный множитель; калькулятор пересчитан на один уровень (оборот сети × ставка статуса + спринт/буст). Доступна из кабинета («Как это работает» → «Открыть презентацию»).
