...
**`apps/smm` — SMM-бот канала @ooplatishka.** Отдельное приложение Dokploy,
отдельный образ, свой том с SQLite. Зависимости намеренно четыре:
`@anthropic-ai/sdk`, `grammy`, `zod`, `pino` — линт, парсеры лент и разбор
витрины канала написаны свои. Кода веб-приложения не знает и не импортирует,
общих пакетов с ним НЕ делит: у бота своя модель, свой провайдер и свой
жизненный цикл. Устройство — `reference/smm-agent.md`.
...
lib/
  ...
  referral/                       захват реферера, начисление, реверс, исполнитель выплат (mock);
                                  spend-math (математика скидки за баллы) + spend (гейты и занятие)
...
Режим бот читает при любом значении `SUPPORT_AI_ENABLED`: помощник — выключатель модели, а не
учёта. «Доступен ли помощник» (флаг И ключ) — одна функция `isSupportAiAvailable()`
(`lib/support/availability.ts`).

Эскалация — четыре источника (жёсткое слово, tool `request_human`, выходной фильтр, авария
или недоступность модели, включая помощника, выключенного посреди живой сессии), один путь
`escalate()`: переход `→ operator` с `mode_expires_at = NULL`
...
Панель: ответ = захват (атомарно, чужой не перебивается), «Вернуть помощнику», «Закрыть». Ручная кнопка «Отвечено» (`markSupportRequestAnswered`) снимает обращение, на которое ответили мимо панели — личкой в Telegram: клиенту ничего не уходит, разговор у оператора освобождается в `idle`.
Крон `support-housekeeping` раз в 15 минут: автозакрытие после суток тишины клиента и алёрт
«без ответа > 2 ч». Правило «обращение ждёт человека» — один SQL-фрагмент
`awaitingOperatorSql` (`packages/db/src/repositories/support-awaiting-sql.ts`) на список,
счётчик меню и этот алёрт. Подробности и инварианты — `CLAUDE.md`, раздел «Поддержка».
...
- **Списание баллов в счёт заказа (2026-09-10, трек referral-balance-spend).** Первый работающий способ потратить баланс: переключатель «Списать баллы» на экране заказа Mini App гасит часть суммы, остальное клиент платит рублями. Флаги `REFERRAL_ENABLED` + `REFERRAL_SPEND_ENABLED` + allowlist `REFERRAL_SPEND_ALLOWLIST`. **Баллы платятся из МАРЖИ заказа** (потолок = комиссия), поэтому рублей всегда хватает на выпуск карты, а денежный путь не тронут; инвариант начисления стал строже — начисления + списание ≤ комиссия. Учёт — `referral_redemptions` (состояние, `order_id` PK, `reserved → spent | released`), математика — один модуль `lib/referral/spend-math.ts` на витрину и на `payments/create`, занятие — под ТЕМ ЖЕ локом `hashtext(userId)`, что заявка на вывод, и по ОБЩЕМУ `balanceExpr` (инлайн-копия формулы в `createReferralPayout` убрана этим же треком). Возврат: `expired`/`cancelled` БЕЗ успешного платежа — правилом (резерв перестаёт вычитаться), `failed` — кнопкой оператора (`/api/panel/orders/bonus-refund`), потому что `failed` не синоним «деньги вернули», а автовозврат оттуда — единственный путь к отрицательному балансу. ⚠️ Оговорка про платёж закрывает `paid_after_terminal` (крон похоронил заказ, оплата пришла следом): без неё клиент получал бы и скидку по оплаченному счёту, и баллы обратно. «Живо ли списание» и «ждёт ли оно решения человека» определяют по одной функции на весь код — `liveRedemptionSql` и `isBonusAwaitingDecision`. Сторож в `referral-recovery` напоминает о `failed` со списанием старше 3 дней; при списании сверяются контакты партнёра с его рефералами (сигнал самореферала, E3-lite). Подробности — `docs/reference/referral-program.md`.

Модель данных: `referral_partners` (профиль/статус), `referral_accruals` (append-only ledger, reversal = новая строка `status='reversed'`; идемпотентность commission — частичный `UNIQUE(payment_id, beneficiary, level) WHERE status='accrued'`, чтобы reversal с теми же ключами проходил), `referral_payouts` (заявки; `method`/`fee_usd_cents`/`destination` — Этап E, nullable, маска PAN без CVV; `transitionReferralPayout` форсит машину статусов в БД-слое), `referral_monthly_stats` (агрегаты прогрессии), `referral_redemptions` (списания баллов: одна строка на заказ, `released_by` — кто вернул руками). Границы пакетов те же: расчёт — `@oplati/types` (чистые функции), доступ к БД — `@oplati/db`, оркестрация/API — `apps/web`.
