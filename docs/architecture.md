...
Модель данных: `referral_partners` (профиль/статус), `referral_accruals` (append-only ledger, reversal = новая строка), `referral_payouts` (заявки; `method`/`fee_usd_cents`/`destination` — Этап E, nullable, маска PAN без CVV), `referral_monthly_stats` (агрегаты прогрессии). Границы пакетов те же: расчёт — `@oplati/types` (чистые функции), доступ к БД — `@oplati/db`, оркестрация/API — `apps/web`.
