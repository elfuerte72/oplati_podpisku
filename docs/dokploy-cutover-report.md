...
закрыты `S-1` (автодеплой Vercel отключён + WAF-deny на кроны), `S-2` (env Sentry заполнен, нужна пересборка), `S-4` (бэкап сделан и проверен восстановлением), `S-8` (расклеена строка env), `A-1`, `A-4`, `B-3`, `D-1`, `D-2`, `D-3`, `E-1`, `E-2`, `E-3`, `E-4`, `E-5`, `E-7`,
...
- [ ] **S-9 · СРЕДНЕ · `recycle-cards`: «простой» считается по НАШИМ топапам, а не по реальным
      списаниям клиента.** Разобрано при объяснении джоба 2026-07-25, к переезду не относится —
      поведение такое с самого начала.
