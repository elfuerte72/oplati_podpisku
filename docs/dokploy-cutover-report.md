...
закрыты `A-1`, `A-4`, `B-3`, `D-1`, `D-2`, `D-3`, `E-1`, `E-2`, `E-3`, `E-4`, `E-5`, `E-7`,
`E-8`, `E-10` + код-сторона `S-1` (блок `crons` снят из `vercel.json`).
После правок: typecheck 4/4, тесты web **457** (было 440), lint 0 ошибок.
Осознанно НЕ правил в этом заходе (нужно решение/доступ владельца): `S-2` (Sentry DSN),
`S-3`/`C-1` (секрет cron в 600-файл — правка на VPS), `S-4` (ручной бэкап), `S-5`/`C-3`
(отдельная роль Postgres), `S-6`/`C-4` (публичный репо), `S-7`, `S-8`, `A-2`, `A-3`, `A-5`,
`B-1` (гаснет вместе с `S-1`), `B-2`, `B-4`, `C-2`, `C-5`, `D-4`, `D-5`, `E-6`, `E-9`.
...
- [ ] **S-1 · КРИТИЧНО · Живой split-brain: Vercel продолжает обслуживать клиентов и крутить cron.**
...
- [ ] **S-2 · ВЫСОКО · Sentry на новом проде выключен — прод без трекинга ошибок.**
...
- [ ] **S-4 · СРЕДНЕ · Свежего бэкапа нового прода ещё нет.** Единственный прогон бэкапа —
      «Initializing Backup» в 15:34 UTC, т.е. ДО финального переноса данных (17:00–17:25).
      Первый штатный дамп — сегодня в 03:00 UTC. До него всё, что записано после cutover, живёт
      только в docker-томе `oplatishka-db-ry3smb-data`. Фикс: прогнать ручной бэкап сейчас.
...
- [ ] **S-8 · НИЗКО · Разное по env/конфигу.** `TELEGRAM_MINIAPP_SHORTNAME` на Dokploy не задан →
      кнопка «Личный кабинет» на сайте уходит через `?start=cabinet`-меню, а не прямой ссылкой
      (регресс UX, не поломка). В env-блоке склеены две переменные в одну строку:
      `PAYSPACE_WEBHOOK_SECRET=<...>DEV_CRON_SECRET=<...>` — функционально безвредно
      (`PAYSPACE_WEBHOOK_SECRET` в коде нигде не читается), но это симптом потерянного переноса строки.
