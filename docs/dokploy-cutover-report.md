...
После правок: typecheck 4/4, lint 0 ошибок, тесты web **457** (было 440), `@oplati/db` **37** (было 32).
...
- [x] **S-9 · ИСПРАВЛЕНО 2026-07-25 (решение владельца: без предупреждений клиенту).**
      ...
      **Осталось открытым — следствие 2 (денежное):** после 90 дней без топапов повторный заказ
      выпускает новую карту вместо долива старой, и клиент снова платит $4
      (`findActiveByUserId` матчит только `active`). Не трогал — требует решения, как считать.
      Исходный разбор:
