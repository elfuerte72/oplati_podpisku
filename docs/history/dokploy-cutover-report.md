...
закрыты `S-1` (автодеплой Vercel отключён + WAF-deny на кроны), `S-10` (деплой из Actions с гейтом CI), `S-11` (разобрано), `S-2` (env Sentry заполнен, нужна пересборка), `S-4` (бэкап сделан и проверен восстановлением), `S-8` (расклеена строка env), `S-9` (жёсткий срок карты 180д + сокрытие в кабинете), `A-1`, `A-4`, `B-3`, `D-1`, `D-2`, `D-3`, `E-1`, `E-2`, `E-3`, `E-4`, `E-5`, `E-7`,
...
- [x] **S-10 · ИСПРАВЛЕНО 2026-07-25 · Автодеплой Dokploy не работал; теперь деплой из Actions.**
      Встроенный GitHub App Dokploy молча потерял мерж PR #102 и #103 — прод пересобирали руками.
      Что установлено проверками: Dokploy→GitHub жив (`github.testConnection` видит 14 репозиториев),
      эндпоинт `POST /api/deploy/github` доступен извне и требует HMAC-подпись
      (`{"message":"Missing signature header"}`), очередь деплоев пуста, логов у контейнера Dokploy
      нет вовсе, репозиторных вебхуков нет (интеграция через App). **Корневую причину со стороны
      репозитория установить нельзя** — остались два кандидата (GitHub не доставляет / Dokploy
      отвергает подпись), различимые только в журнале доставок самого App: GitHub → App
      `dokploy-2026-04-26-j4eqtl` → Advanced → Recent Deliveries.
      **Починили заменой, а не отладкой App** — потому что схема была плохой независимо от бага:
      App деплоил СРАЗУ на push, не дожидаясь CI, то есть красный `main` уехал бы на боевой контур
      с живыми платежами без гейта, и его отказы не видны из репозитория.
      Теперь единственный триггер — `.github/workflows/deploy.yml`: push в `main`/`dev` → gate
      (typecheck + тесты + lint) → `POST /api/deploy/<refreshToken>`. `autoDeploy` выключен у ОБОИХ
      приложений (иначе при оживании App-вебхука была бы двойная сборка, причём одна — без гейта).
      Проверено сквозным прогоном: мерж PR #105 → гейт зелёный → `200 Application deployed
      successfully` → прод пересобрался сам, контейнер healthy, конфигурация приложения цела
      (33 env, buildArgs, домен, ветка, токен).
      **Контракт вебхука (в доках Dokploy его НЕТ, снят перебором):** нужен заголовок
      `X-GitHub-Event: push` И тело `{"ref":"refs/heads/<ветка>"}`; без заголовка ветка не
      извлекается → `301 Branch Not Match`; `{"branch":"dev"}` не подходит; неизвестный токен →
      `404 Application Not Found`; подпись HMAC этому роуту не нужна.
      **Грабля инструментов:** `gh secret set --body -` НЕ читает stdin, а пишет литерал `-` —
      из-за этого первый прогон получил 404. Задавать через `gh secret set NAME < file`.
      Побочная польза: у ветки `dev` до этого не было CI вообще — теперь она под тем же гейтом.
      **Вторая грабля, найденная на живом проде:** флаг `autoDeploy` у приложения — это НЕ «слушать
      GitHub App», а общий выключатель ВЕБХУК-деплоев. Выключив его ради единственности триггера, я
      обрубил и собственный workflow: Dokploy стал отвечать
      `400 {"message":"Automatic deployments are disabled for this application"}`. Флаг возвращён в
      `true` у обоих приложений — он ОБЯЗАН быть включён. Единственность триггера держится на том,
      что App-вебхук события не доставляет; а даже если оживёт, ungated-сборка `main` не страшна —
      прямой push в main запрещён ruleset'ом, туда попадает только PR с зелёными
      `Tests`/`Type Check`/`Lint`.

- [x] **S-11 · РАЗОБРАНО 2026-07-25 · Sentry-алёрты в Telegram: дело не в секрете.**
      Проверил правила проекта: единственное правило «Send a notification for high priority issues»
      отправляет **Email** issue-owners'ам. **Webhook-действия в нём нет вообще** — то есть
      `SENTRY_ALERT_WEBHOOK_SECRET` и эндпоинт `/api/alerts/sentry` ни к чему не подключены, и
      «подставить секрет в URL» нечего: действие нужно СОЗДАТЬ. Секрет в env прода уже лежит.
      Сделать это через доступные мне инструменты нельзя: Sentry MCP на правила только читающий
      (`find_alert_rules`/`get_alert_rule`), инструмента создания/изменения в каталоге нет —
      только UI. Ошибки при этом в Sentry долетают (проверено, п. 4.11), а прямые ops-DM
      (`notifyOps`: proxy-health, недоплаты) работают через свой бот, минуя Sentry.
