...
Там же до 2026-09-22 жил SMM-агент Гилфойл (Hermes, профиль `smm`); выведен, установка и профиль
перенесены в `/root/archive/hermes-20260922/` (CHANGELOG 2026-09-22). С 2026-09-05 у ВМ
(Hostinger id 1621087) есть edge-файрвол `oplatishka-bos-agent` (TCP 22/80/443, UDP 443, ICMP) —
до этого она жила только на ufw хоста, который опубликованные докером порты обходят.
...
⚠️ **Выровнять ветку — не значит пересобрать стенд.** Пуш ref'а токеном `GITHUB_TOKEN` по правилу
GitHub (защита от рекурсии) НЕ запускает workflow, поэтому `deploy.yml` для `dev` после
выравнивания не стартовал, и dev-стенд с 2026-07-30 жил на образе последнего РУЧНОГО пуша в
`dev` — при `dev = main` по коду. Замечено 2026-08-28 по `startedAt` от 19.08 и `migrations_ahead`
в `/api/ready` стенда. Единственное исключение из правила — `workflow_dispatch`: с 2026-08-28 шаг
после переноса ref'а зовёт `gh workflow run deploy.yml --ref dev` (job'у выдано `actions: write`),
и dev пересобирается своим прогоном с полным гейтом. Если запуск не удался — `::warning::` и
ручной `gh workflow run deploy.yml --ref dev`.
