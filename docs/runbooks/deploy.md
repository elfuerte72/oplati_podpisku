...
## Выкат панели v2 (трек admin-panel-v2)

У этого трека есть шаг, которого нет ни у одного другого: **роль в БД**. Панель
работает и без него — раздел «Аналитик» честно пишет «не настроено», — поэтому
шаги 2-3 можно делать отдельно от выката кода.

1. **Миграции 0043 и 0044** — ДО мержа. Обе только ДОБАВЛЯЮТ (таблицы
   `funnel_texts`, `funnel_text_revisions` и append-only триггер), старый образ
   их не спрашивает, поэтому порядок «миграции → мерж» безопасен и оставляет
   `/api/ready` зелёным сразу после выката. Журнал: 43 → 45. Каждый файл
   применять вместе с записью журнала В ОДНОЙ транзакции (`psql -1`): иначе
   упавшая на середине миграция оставит журнал расходящимся со схемой.
2. **Роль `panel_ai_ro`** — `packages/db/scripts/panel-ai-role.sql` с настоящим
   паролем (`openssl rand -hex 24`) через `psql` в контейнере БД; файл
   идемпотентен, пароль перевыставляется отдельным `ALTER ROLE`. Проверка
   ВЖИВУЮ, а не «команда прошла»: заказы и `client_feedback` читаются,
   `users.email` / `staff.totp_secret` / `payments.raw_payload` /
   `vpn_subscriptions.subscription_url` / `messages` — `permission denied`,
   `UPDATE` — `cannot execute UPDATE in a read-only transaction`.
3. **`PANEL_AI_DATABASE_URL`** в env приложения Dokploy (порядок с бэкапом и
   построчной сверкой — раздел про env выше) + редеплой, чтобы контейнер увидел
   переменную. ⚠️ MCP отдаёт env замаскированным (`[REDACTED]`) — сохранять
   таким значением нельзя, `saveEnvironment` перезапишет блок целиком и снесёт
   все переменные; читать блок нужно API с самого VPS.
4. **Смоук:** все `/admin/*` → 307 на вход с `admin.oplatishka.com`, публичные
   домены (`www`, `new`) → 404 на `/admin`, `/api/ready` — ok.

**Состояние на 2026-09-02:** шаги 1-4 выполнены на ПРОДЕ (журнал 45, роль
проверена вживую, env задан, смоук зелёный). На **dev** роли и переменной нет —
раздел «Аналитик» там отвечает «не настроено»; из-за этого `eval:panel-ai`
гонялся только против локальной копии контура (`docs/BACKLOG.md`).

Откат — убрать `PANEL_AI_DATABASE_URL` + redeploy: раздел «Аналитик» вернётся к
«не настроено», остальная панель не затрагивается. Миграции откатывать не нужно
(пустые таблицы безвредны); роль можно оставить — она read-only.

---

## Бот SMM (`oplatishka-smm`)

Отдельное приложение Dokploy на том же VPS. Собирается из `apps/smm` только с
ветки `main`; dev-стенда у бота нет — тестовый канал задаётся переменными
окружения самого приложения.

### Как задеплоить

Обычный мерж в `main`. Job `deploy-smm` в `deploy.yml` срабатывает, только
если в пуше есть файлы под `apps/smm/**` (или общие: корневые конфиги,
lock-файл, workflow). Триггер — тот же вебхук Dokploy, что у веба, токен в
секрете `DOKPLOY_SMM_WEBHOOK`.

⚠️ **Подтвердить выкат снаружи нечем:** бот не поднимает HTTP (long polling, а
не webhook), а `getMe` отвечает одинаково у любой версии. Job подтверждает
принятый триггер, а доказательство читается в логе контейнера.

⚠️ **`gh secret set --body -` записывает в секрет буквальный дефис.** У этой
команды `--body` принимает СТРОКУ, а не stdin; чтобы положить значение из
конвейера, нужно перенаправление: `gh secret set ИМЯ --repo … < файл`. Ошибка
тихая — `gh` отвечает «успешно», а деплой потом падает с
`404 {"message":"Application Not Found"}`, потому что панель получает запрос на
`/api/deploy/-`. Поймано 22.09.2026 первым же выкатом бота: два прогона подряд
дали один и тот же 404, хотя тот же токен из панели руками работал.

⚠️ **Watch paths у приложения в Dokploy обязаны быть ПУСТЫ.** Панель проверяет
их по списку изменённых файлов из тела вебхука, а workflow шлёт только
`{"ref": …}`: с заполненными путями ответ будет
`301 {"message":"Watch Paths Not Match"}`, то есть сборка не стартует, а job
увидит 3xx и сочтёт это ошибкой конфигурации. Проверено живым вызовом
22.09.2026. Нужный фильтр уже сделан job'ом `changes` в пайплайне.

Проверить контракт вебхука, НЕ запуская сборку, можно заведомо чужой веткой:

```bash
curl -sS -X POST "$PANEL/api/deploy/$TOKEN" \
  -H 'X-GitHub-Event: push' -H 'Content-Type: application/json' \
  -d '{"ref":"refs/heads/несуществующая"}'
# → 301 {"message":"Branch Not Match"} — токен верный, маршрут живой
```

### Как посмотреть логи

Панель Dokploy → `oplatishka-smm` → Logs. Или по ssh:

```bash
ssh root@187.124.172.104 'docker service logs --tail 200 $(docker service ls --filter name=oplatishka-smm -q)'
```

Признак живого старта — строки «бот слушает», «тикер источников запущен» и
«проверка здоровья завершена». Логи структурные (pino): `postId`, `role`,
`usdMicros` — поля, а не текст сообщения. Ключи и тела постов в лог не
попадают (редактируются при записи, канарейка в `src/logger.test.ts`).

### Как проверить здоровье

Сторож ходит сам при старте и раз в час; красное уходит в тему «Ошибки»
ops-группы от бота входа. Руками — написать боту `/stats`: отчёт отвечает
только живой процесс с доступной базой.

### Том с базой

SQLite лежит на томе приложения по пути `SMM_DB_PATH` (по умолчанию
`data/smm.db`), рядом каталог `media` с обложками. В нём — посты, журнал
решений владельца, идеи из источников, расход на модель и снимки просмотров.

Бэкап: Dokploy → приложение → Volumes → Backup (тот же механизм, что у
«Бэкапы БД» основного контура). ⚠️ Перед ручным копированием файла бота лучше
остановить: SQLite в режиме WAL копируется целиком только вместе с `-wal` и
`-shm`, иначе копия окажется на состоянии последнего чекпойнта.

Восстановление: положить файлы обратно на том и перезапустить приложение.
Миграции forward-only, поэтому база от СТАРШЕГО образа на младшем не
поднимется — см. [`rollback.md`](rollback.md).
