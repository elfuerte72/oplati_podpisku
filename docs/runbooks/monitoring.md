...
## Алёрты персоналу в личку (бот входа, `notifyStaff`)

Отдельный от Grafana канал: пишет бот входа в панель, получатели — строки
`staff` с правом на соответствующий раздел; если получателей нет, уведомление
уходит владельцу (`notifyOps`). Про деньги и клиентов сигналят сюда, а не в
Grafana, потому что это требует действия человека, а не наблюдения за графиком.

| Сообщение | Окно | Что значит |
|---|---|---|
| «Клиент не смог оплатить …» | 15 мин | карточного фонда не хватило, клиенту отказано. **Деньги НЕ приняты**, заказ жив — пополнить счёт (`vcc-funding.md`) |
| «Критически мало на карточном счёте …» | 1 ч | следующий типовой заказ упадёт при выпуске |
| «На карточном счёте … на самый дорогой не хватает» | 1 сутки | норма для нашего остатка, повод свериться с таблицей в `vcc-funding.md` |
| «Не удалось прочитать карточный счёт …» | 1 ч | ⚠️ **проверка сейчас не защищает**: баланс не прочитан, оплаты идут без неё (fail-open) |
| «Не удалось занять карточный фонд …» | 15 мин | клиенту отказано из-за очереди расчётов (fail-closed). На спокойном трафике — смотреть длинные транзакции по заказам |
| «Проверка отключилась: в базе нет её таблиц» | 1 ч | ⚠️ миграции 0039/0040 не применены на этой базе; гейт молчит, оплаты идут как до трека |
| «Заказ … застрял после оплаты» | по заказу | автоповтор выпуска не помог, нужна ручная проверка |
| «Платёж на проверке банка: операция …» | 1 ч по платежу | антифрод-холд Freekassa (статус 7): заказ в `/admin/holds` («Проверка платежей»), исход решает провайдер |
| «Новое обращение в поддержку» | — | `/admin/support` |
...
| `ALERT_TELEGRAM_CHAT_ID` | `379336096` — куда шлём алёрты |
