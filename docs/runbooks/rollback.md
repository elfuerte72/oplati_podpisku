...
3a. ⚠️ **Поднять обратно squid-прокси к Love&Pay** — иначе платежи не заработают:
   ...
   ```bash
   ssh root@187.124.172.104 'bash /opt/lnp-proxy/run.sh'   # поднимает squid на :24128
   ```
...
| ~~**squid-прокси** `187.124.172.104:24128`~~ | **удалён 2026-07-25** — приложение выходит в L&P напрямую с того же IP (проверено: их API отвечает `401 MISSING_HEADERS`, не `403 SOURCE_IP_NOT_ALLOWED`) | конфиг оставлен в `/opt/lnp-proxy/`, поднять обратно: `bash /opt/lnp-proxy/run.sh` |
