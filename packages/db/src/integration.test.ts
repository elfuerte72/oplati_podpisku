...
describe('панель: поддержка (тикет 10)', () => {
  ...
  it('обращение флоу без режима (помощник выключен) ждёт человека до ответа', async () => {
    ...
    // помощник — не эскалация). Снимает его ответ или «Подключиться» →
    // «Закрыть» (сценарии — `support.integration.test.ts`, тикет 03), а до тех
    // пор счётчик обязан его видеть, иначе в режиме деградации панель слепнет.
    const before = await countUnansweredSupportRequests(db);
