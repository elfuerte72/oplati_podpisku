...
/**
 * Заказы, которые ЗАНИМАЮТ карточный фонд, — обязательства (трек
 * vcc-preflight, тикет 04).
 *
 * ⚠️ Имя именно про фонд, а не про «ждут выпуска карты»: в выборку входят и
 * `pending_payment`, за которые ещё не заплатили. Карту им никто не ждёт, но
 * деньги под них зарезервированы — переиспользовать эту функцию для recovery
 * выпуска нельзя, она вернёт неоплаченные заказы.
 *
 * Гейт оплаты отвечает не на вопрос «сколько лежит на счёте», а «сколько
 * свободно»: пока другой клиент держит живой счёт на всю сумму фонда, новый
 * заказ провести нечем, хотя деньги формально на месте.
 *
 * Кто считается обещанным:
 *   - `pending_payment` с ЖИВЫМ счётом — деньги могут прийти в любую минуту;
 *   - `paid` — деньги пришли, выпуск не начат;
 *   - `in_fulfillment` — выпуск идёт прямо сейчас.
 *
 * ⚠️ `ready_for_payment` в выборку НЕ входит, и это не упущение: именно в этом
 * статусе лежит заказ, ради которого гейт и считает. Добавь его сюда — и заказ
 * начнёт вычитать сам себя, получая отказ там, где денег хватает.
 *
 * ⚠️ Счёт БЕЗ срока считается живым. `expires_at` проставляется при выставлении
 * счёта всегда, поэтому пустое поле — аномалия; считать такой заказ свободным
 * значило бы занизить обязательства и пропустить оплату, которую нечем
 * исполнить. Ошибка уходит в безопасную сторону.
 *
 * ⚠️ Условия — обычные builder-выражения, а не сырой `sql`: `Date` в
 * raw-фрагменте роняет postgres-js на проде и незаметен на PGlite в тестах
 * (инцидент 2026-08-15).
 *
 * ⚠️ **Зеркало** (инвариант 10): «живой счёт» здесь определяется по
 * `orders.expires_at`, а панель недожатых заказов смотрит на `payments` со
 * статусом `pending` и их срок. Совпадают они только потому, что оба поля
 * выравниваются в ОДНОЙ транзакции в `payments/create`; разъедутся — гейт и
 * панель начнут считать живыми разные счета.
 *
 * ⚠️ Заказ, залипший в `paid`/`in_fulfillment` (о таких кричат
 * `findStuckPaidOrders` и `findStuckInFulfillmentOrders`), продолжает занимать
 * фонд, пока его не разберут руками. Это осознанно: пока recovery-крон
 * повторяет выпуск, карта по нему всё ещё может уйти, и «освободить» такие
 * деньги значило бы пропустить оплату, которую нечем исполнить.
 *
 * Лимита нет намеренно. Усечение занизило бы обязательства — то есть ошиблось
 * бы в ОПАСНУЮ сторону, пропустив заказ, который нечем исполнить. Строк тут
 * единицы: два статуса транзиентные, третий живёт час.
 */
export async function findOrdersCommittingCardFund(
  db: DBLike,
  now: Date = new Date(),
): Promise<{ id: string; originalAmount: number | null }[]> {
  // `id` отдаётся ради диагностики: по нему видно, какой именно заказ держит
  // фонд, когда гейт начинает отказывать без видимой причины.
  return await db
    .select({ id: orders.id, originalAmount: orders.originalAmount })
    .from(orders)
    .where(
      or(
        inArray(orders.status, ['paid', 'in_fulfillment']),
        and(
