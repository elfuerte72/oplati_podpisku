...
// ─── Снимок карточного фонда (PaySpace) ───────────────────────────────────
//
// Остаток VCC-субаккаунта, увиденный кроном. Заведён для preflight
// (`.scratch/vcc-preflight/`, тикет 03): гейт оплаты обязан знать, есть ли чем
// выпустить карту, но ходить за этим к провайдеру в горячем пути — значит
// добавить клиенту чужую задержку и чужую недоступность.
//
// Почему снимок не устаревает опасно: баланс меняется от ДВУХ причин — наши же
// выпуски карт (видны в наших заказах, мгновенно) и пополнения владельцем
// (редкие, идут T+1). Значит «снимок пятиминутной давности» ошибается только в
// безопасную сторону: траты учтены сразу, пополнение — с задержкой до прогона
// крона, и цена ошибки — один лишний отказ, который пройдёт через пять минут.
//
...
export const vccBalanceSnapshots = pgTable('vcc_balance_snapshots', {
  ...
  // эмитент не должен требовать миграции, а перепутать их строки нельзя.
  provider: text('provider').primaryKey(),
  // ДОСТУПНЫЙ остаток. ⚠️ Не сумма с `pending`: замороженными деньгами карту
  // не профинансировать, и пропущенный по ним заказ упал бы на выпуске.
  balanceUsdCents: integer('balance_usd_cents').notNull(),
  // Замороженная часть — только для наблюдения (панель, разбор инцидентов).
  pendingUsdCents: integer('pending_usd_cents').notNull(),
  // Момент ОПРОСА провайдера, а не записи в базу: по нему считается свежесть,
  // и отметка «сейчас» при старом ответе означала бы ложную свежесть. Пишется
  // время ДО вызова — так снимок считается на доли секунды старше, чем есть,
  // то есть ошибается в безопасную сторону.
  readAt: timestamp('read_at', { withTimezone: true }).notNull(),
}).enableRLS();

// ─── Резервы карточного фонда ─────────────────────────────────────────────
//
// «Занятые» деньги под заказ, которому вот-вот выставят счёт (трек
// vcc-preflight, тикет 05). Заведены под ОДНО окно, которое подсчёт по статусам
// не закрывает: заказ становится `pending_payment` не в момент прохождения
// гейта, а ПОСЛЕ создания счёта у платёжного шлюза — между этими событиями
// внешний вызов длиной от полусекунды до пары секунд. Двое, нажавшие
// «Оплатить» внутри окна, оба видели пустую очередь и оба получали счёт при
// фонде на одного.
//
// ⚠️ Одна строка на заказ (`order_id` первичным ключом), а не журнал занятий:
// повторное нажатие того же клиента обязано занимать деньги ОДИН раз, и это
// гарантируется ключом, а не проверкой в коде.
//
// ⚠️ Резерв учитывается, только пока его заказ в `ready_for_payment`. Ушёл в
// `pending_payment` — деньги считаются уже по статусу (обязательства, тикет
// 04), и учитывать их дважды нельзя. Отдельного «снятия» для этого не нужно:
// строка просто перестаёт влиять и протухает по сроку.

export const vccFundReservations = pgTable(
  'vcc_fund_reservations',
  {
    orderId: uuid('order_id')
      .primaryKey()
      .references(() => orders.id, { onDelete: 'cascade' }),
    // Сколько фонда занято под этот заказ (цена + буфер + комиссия за выпуск).
    amountUsdCents: integer('amount_usd_cents').notNull(),
    // Срок занятия = сроку счёта. Умер процесс между занятием и созданием счёта —
    // деньги освободятся сами, максимум через час, без крона и без ручного шага.
    expiresAt: timestamp('expires_at', { withTimezone: true }).notNull(),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
  },
  (t) => ({
    // ⚠️ Индекс не «на всякий случай»: выборка живых занятий выполняется ВНУТРИ
    // глобального замка фонда, и её время — это время, на которое встают все
    // оплаты сразу. Строки на успехе не удаляются (заказ ушёл к оплате, резерв
    // просто перестал влиять), поэтому таблица растёт, а скан обязан не расти
    // вместе с ней.
    expiresAtIdx: index('vcc_fund_reservations_expires_at_idx').on(t.expiresAt),
  }),
).enableRLS();
