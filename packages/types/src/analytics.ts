 ...
 * подписи в панели (воронка «Отчётов», лента действий клиента) и словарь
 * `analytics_event_types` в БД (наполняется upsert'ом из этого файла) — справочник
 * для того, кто в код не смотрит. Дрейф между кодом и подписями поэтому невозможен.
 *
