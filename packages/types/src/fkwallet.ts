 ...
 *
 * **Подтверждено живым вызовом 2026-09-16** (`GET /balance` из прод-контейнера,
 * HTTP 200): `status` лежит на ВЕРХНЕМ уровне без обёртки, `data` — список по
 * всем валютам кошелька (RUB, USDT, USD, EUR, UAH, KZT, BTC, LTC, TON, ETH, TRX,
 * BNB), `value` — ЧИСЛО (`0.33`). Схема доки при этом описывает `data`
 * объектом и ответ с обёрткой `{ data: { status, data } }` — Zod продолжает
 * принимать и эти формы: дока уже разошлась с живым ответом один раз, и
 * сужать схему до факта значило бы ловить следующий дрейф пятисоткой экрана.
 */
