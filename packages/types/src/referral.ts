...
/**
 * Базовая ставка партнёра (bps) — ЕДИНСТВЕННЫЙ источник дефолта.
 *
 * Применяется из двух мест: дефолт колонки `referral_partners.locked_rate_l1_bps`
 * (профиль создаёт месячный крон прогрессии) и расчёт начисления, когда профиля
 * ещё нет — до первого прогона крона его нет ни у одного нового партнёра. Раньше
 * это были два независимых числа, совпадавших по случайности: разъезд означал бы,
 * что партнёру начисляют не ту ставку, которую показывает кабинет, и молча.
 * Равенство с фактическим дефолтом в БД форсит тест на применённых миграциях.
 */
export const DEFAULT_REFERRAL_RATE_L1_BPS = REFERRAL_RATE_TABLE[REFERRAL_DEFAULT_CIRCLE]!.l1Bps;
